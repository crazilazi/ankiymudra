...
class KoinList extends React.Component<IKoinListProps, IKoinListState> {
    ...
    public getPageddata = (index: number): IRowData[] => {
        const tickers = this.allMudra.slice((index * this.state.pageSize), ((index + 1) * this.state.pageSize));
        const noOfRows: number = Math.ceil(tickers.length / 12);
        const rowData: IRowData[] = [];
        for (let i = 0; i < noOfRows; i++) {
            const temp: IRowData = { Row: i, Tickers: tickers.slice((i * 12), (i + 1) * 12) };
            rowData.push(temp);
        }
        return rowData;
    }
    ...
    public render = () => {
        ...
        const hide = !this.state.isPageLoaded ? 'd-none' : 'container-fluid mt-2';

        return (
            <div id="App">
                <SideNavBar refreshClick={this.getAllTicker} />
                <div className={hide} style={{ width: '90%' }}>
                    <div >
                        <div className="row">
                        ...
                        {this.state.pagedData.map(rowData => (
                            <div key={rowData.Row} className="row">
                                {rowData.Tickers.map(koin => (
                                    <div className="col-xs-1 col-sm-1 col-md-1 col-lg-1 col-xl-1" key={koin.id} style={{ marginBottom: 2 }}>
                                        <div className={"card " + this.getClassBasedOnDropdown(koin.quotes.USD)} style={{ minWidth: '92px' }}   >
